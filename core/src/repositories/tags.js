...
exports.create = async (tagName, result) => {
  await TagsModelSequelize.create({
    tagname: tagName.tagname,
    description: tagName.tagname,
  }).then((response) => {
    result(
      null,
      responseHandler(true, 200, 'Tag Created', response.id),
    );
  })
    .catch((error) => {
      console.log(error);
      result(responseHandler(false, 500, 'Some error occurred while creating tag.', null), null);
    });
};
