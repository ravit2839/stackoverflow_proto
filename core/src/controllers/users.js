...
exports.allUsers = asyncHandler(async (req, res) => {
  ...
  const users = await MongoUser.find(keyword).find({ _id: { $ne: req.user._id } });
  res.send(users);
});

exports.userTags = asyncHandler(async (req, res) => {
  try {
    const { id } = req.params;
    await usersService.userTags(
      id,
      (err, data) => {
        if (err) {
          console.log(err);
...
exports.userAnswers = asyncHandler(async (req, res) => {
  try {
    const { id } = req.params;
    await usersService.userAnswers(
      id,
      (err, data) => {
        if (err) {
          console.log(err);
